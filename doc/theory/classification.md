...
For any risk level \alpha \in (0, 1), the methods allow constructing a prediction set \hat{C}*{n, \alpha}(X*{n+1}) with a **marginal coverage guarantee**:


P Y_{n+1} \in \hat{C}*{n, \alpha}(X*{n+1})  \geq 1 - \alpha
...
!!! info
    The guarantee applies only to **marginal coverage**, not conditional coverage P Y_{n+1} \in \hat{C}*{n, \alpha}(X*{n+1}) | X_{n+1} = x_{n+1}  which depends on the location of the test point.
...
s_i(X_i, Y_i) = j \quad \text{where} \quad Y_i = \pi_j \quad \text{and} \quad \hat{\mu}(X_i)*{\pi_1} > \cdots > \hat{\mu}(X_i)*{\pi_n}
...
\hat{C}(X_{\text{test}}) = \pi_1, \ldots, \pi_k \quad \text{where} \quad k = \inf\leftk : \sum^k_{j=1} \hat{\mu}(X_{\text{test}})_{\pi_j} \geq \hat{q}\right
...
s_i(X_i, Y_i) = \sum^k_{j=1} \hat{\mu}(X_i)*{\pi_j} + \lambda (k - k*{\text{reg}})^+ \quad \text{where} \quad Y_i = \pi_k
...
\hat{C}(X_{\text{test}}) = \pi_1, \ldots, \pi_k \quad \text{where} \quad k = \inf\leftk : \sum^k_{j=1} \hat{\mu}(X_{\text{test}})*{\pi_j} + \lambda(k - k*{\text{reg}})^+ \geq \hat{q}\right
...
1. Define V_i = \frac{s_i(X_i, Y_i) - \hat{q}*{1-\alpha}}{\hat{\mu}(X_i)*{\pi_k} + \lambda \mathbb{1}(k > k_{\text{reg}})}
