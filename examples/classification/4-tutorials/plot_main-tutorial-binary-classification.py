...
# sets transition from being non-empty to potentially empty.
...
conf_level = [
    np.round(confidence_level_[idx-1], 2),
    np.round(confidence_level_[idx+1], 2)
]
calib = CalibratedClassifierCV(
    estimator=clf, method='sigmoid', cv='prefit'
...
calib.fit(X_c1, y_c1)
mapie_clf = SplitConformalClassifier(
    estimator=calib, confidence_level=conf_level, prefit=True, random_state=42
)
mapie_clf.conformalize(X_c2, y_c2)
...
plot_prediction_set(
    y_ps[:, :, 0], np.round(confidence_level_[idx-1], 3), axs[1]
)
_, y_ps = mapie_clf.predict_set(X_test)
plot_prediction_set(
    y_ps[:, :, 1], np.round(confidence_level_[idx+1], 3), axs[2]
)

plt.show()

##############################################################################
# In the code above, the ``idx`` variable identifies the first index where the
# proportion of non-empty prediction sets for the test samples drops below 1. This
# indicates the transition from consistently non-empty prediction sets to potentially
# empty ones.
...
# transition occurs at a confidence_level of 0.89, which means that for confidence
# levels above 0.89, all prediction sets are non-empty. Conversely, for confidence
# levels below 0.89, some test samples may have empty prediction sets.
