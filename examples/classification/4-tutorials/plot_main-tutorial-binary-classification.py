...
mapie_clf = MapieClassifier(estimator=calib, method="lac", cv="prefit", random_state=42)
...
coverage = np.array(
    [classification_coverage_score(y, y_ps_mapie[:, :, i]) for i, _ in enumerate(alpha_)]
)
mean_width = [classification_mean_width_score(y_ps_mapie[:, :, i]) for i, _ in enumerate(alpha_)]
...
non_empty = np.mean(np.any(mapie_clf.predict(X_test, alpha=alpha_)[1], axis=1), axis=0)
