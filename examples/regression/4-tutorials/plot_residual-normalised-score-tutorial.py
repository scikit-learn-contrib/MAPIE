...
STRATEGIES = {
    ...
    "Base model prefit": {
        ...
        "conformity_score": ResidualNormalisedScore(split_size=0.5, random_state=random_state),
    },
    "Base and residual model prefit": {
        "cv": "prefit",
        "estimator": base_model,
        "conformity_score": ResidualNormalisedScore(
            residual_estimator=residual_estimator, random_state=random_state, prefit=True
        ),
    },
    "Wrapped residual model": {
        ...
        "estimator": base_model,
        "conformity_score": ResidualNormalisedScore(
            residual_estimator=wrapped_residual_estimator, random_state=random_state, prefit=True
        ),
    },
}
...
def plot_predictions(y, y_pred, intervals, coverage, cond_coverage, ax=None):
    ...
    ax.set_title(f"{strategy} - coverage={coverage:.0%} " + f"- max violation={cond_coverage:.0%}")
