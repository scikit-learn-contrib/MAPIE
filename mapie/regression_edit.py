...
class MapieRegressor(BaseEstimator, RegressorMixin):
    ...
    def _check_estimator(
        ...
        if (self.method == ("quantile" or "quantile cv")):
            if "quantile" not in estimator.loss:
                raise ValueError(
                    "You need to set the loss method to quantile"
                )
        return estimator
    ...
    def predict(
        ...
        # How do we do if we have like an array? And how could we have an
        # array at this point?
        ...
        # if self.method in ["naive", "base", "quantile"] or self.cv == "prefit":
            # self.conformity_scores_ = np.full(
            #     shape=(3, n),
            #     fill_value=self.conformity_scores_
            # )
            ...
            #         self.conformity_scores_[1], q, method="higher"
            #     )
            # # print("conformity scores")
            # # print(self.conformity_scores_)
            # print("q", q)
