    ...
    if (method == "rcps") and (delta is not None):
        if bound == "hoeffding":
            ...
            for batch in batches:
                ...
                nu_batch = np.repeat(np.expand_dims(nu_batch, axis=2), n_lambdas, axis=2)
                losses_batch = np.repeat(np.expand_dims(losses_batch, axis=2), n_lambdas, axis=2)

                R = lambdas
                K_R = np.cumsum(
                    np.log((1 - nu_batch * (losses_batch - R)) + np.finfo(np.float64).eps), axis=0
                )
...
def find_lambda_star(lambdas: NDArray, r_hat_plus: NDArray, alpha_np: NDArray) -> NDArray:
    ...
    lambdas_star = lambdas[np.argmin(-np.greater_equal(bound_rep, alphas_np).astype(int), axis=1)]
    return lambdas_star
