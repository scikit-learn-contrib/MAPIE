    ...
    if (method == "rcps") and (delta is not None):
        if bound == "hoeffding":
            ...
            sigma_hat = np.concatenate(
                [
                    np.full((1, n_lambdas), fill_value=sigma_init),
                    sigma_hat[:-1],
                ]
            )
            nu = np.minimum(
                1, np.sqrt((2 * np.log(1 / delta)) / (n_obs * sigma_hat))
            )
            ...
            r_hat_plus_tronc = lambdas[
                np.argwhere(np.cumsum(K_R_max > -np.log(delta), axis=1) == 1)[
                    :, 1
                ]
            ]
...
def find_lambda_star(
    ...
    bound_rep = np.repeat(
        np.expand_dims(r_hat_plus, axis=0), len(alphas_np), axis=0
    )
