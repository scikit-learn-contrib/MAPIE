...
SSC_CLASSIF_COVERAGES = {
    ...
    "2alpha_None": np.array([[0.0, np.nan, 0.5, 1.0, 1.0], [0.0, 1.0, 0.0, 1.0, np.nan]]),
}
...
def test_classification_y_true_shape() -> None:
    ...
    with pytest.raises(ValueError, match=r".*y should be a 1d array*"):
        classification_coverage_score(np.tile(y_true_class, (2, 1)), y_pred_set)
    with pytest.raises(ValueError, match=r".*y should be a 1d array*"):
        classification_ssc(np.tile(y_true_class, (2, 1)), y_pred_set_2alphas)
    with pytest.raises(ValueError, match=r".*y should be a 1d array*"):
        classification_ssc_score(np.tile(y_true_class, (2, 1)), y_pred_set_2alphas)
...
def test_top_label_same_result() -> None:
    ...
    """
    pred_proba_ = np.array([[0.2, 0.2, 0.4], [0.5, 0.3, 0.2], [0, 0.4, 0.6], [0.1, 0.7, 0.2]])
...
def test_classification_ssc_score_coverage_values(params: str) -> None:
    """Test that the conditional coverage values returned are correct."""
    cond_cov_min = classification_ssc_score(y_true_class, **SSC_CLASSIF[params])
    np.testing.assert_allclose(cond_cov_min, SSC_CLASSIF_COVERAGES_SCORE[params])
...
def test_regression_coverage_v1andv2() -> None:
    ...
    """
    cov_v1 = regression_coverage_score(y_toy, intervals[:, 0, 0], intervals[:, 1, 0])
...
def test_regression_coverage_score_v2_intervals_invalid_shape() -> None:
    ...
    with pytest.raises(ValueError):
        regression_coverage_score_v2(np.expand_dims(y_toy, 1), intervals[:, 0, 0])
...
def test_classification_coverage_v1andv2() -> None:
    ...
    cov_v2 = classification_coverage_score_v2(
        np.expand_dims(y_true_class, axis=1), np.expand_dims(y_pred_set, axis=2)
    )
    np.testing.assert_allclose(cov_v1, cov_v2[0])
...
def test_classification_coverage_score_v2_ypredset_invalid_shape() -> None:
    ...
    with pytest.raises(ValueError):
        classification_coverage_score_v2(np.expand_dims(y_true_class, axis=1), y_pred_set[:, 0])
