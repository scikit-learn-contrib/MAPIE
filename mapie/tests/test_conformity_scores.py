...
def test_absolute_conformity_score_get_estimation_distribution(
    ...
    abs_conf_score = AbsoluteConformityScore()
    y_obs = abs_conf_score.get_estimation_distribution(X_toy, y_pred, conf_scores)
...
def test_absolute_conformity_score_consistency(y_pred: NDArray) -> None:
    ...
    abs_conf_score = AbsoluteConformityScore()
    signed_conf_scores = abs_conf_score.get_signed_conformity_scores(X_toy, y_toy, y_pred)
    y_obs = abs_conf_score.get_estimation_distribution(X_toy, y_pred, signed_conf_scores)
    np.testing.assert_allclose(y_obs, y_toy)
...
def test_gamma_conformity_score_get_estimation_distribution(
    ...
    gamma_conf_score = GammaConformityScore()
    y_obs = gamma_conf_score.get_estimation_distribution(X_toy, y_pred, conf_scores)
    np.testing.assert_allclose(y_obs, y_toy)
...
def test_gamma_conformity_score_check_oberved_value(y_pred: NDArray, y_toy: NDArray) -> None:
    """Test methods consistency for GammaConformityScore."""
    gamma_conf_score = GammaConformityScore()
...
def test_gamma_conformity_score_check_predicted_value(
    ...
    """Test methods consistency for GammaConformityScore."""
    gamma_conf_score = GammaConformityScore()
    with pytest.raises(ValueError, match=r".*At least one of the predicted target is negative.*"):
        gamma_conf_score.get_signed_conformity_scores(X_toy, y_toy, y_pred)
    with pytest.raises(ValueError, match=r".*At least one of the predicted target is negative.*"):
        gamma_conf_score.get_estimation_distribution(X_toy, y_pred, conf_scores)
...
def test_check_consistency() -> None:
    ...
    conformity_scores = dummy_conf_score.get_signed_conformity_scores(X_toy, y_toy, y_pred_list)
    with pytest.raises(ValueError, match=r".*The two functions get_conformity_scores.*"):
        dummy_conf_score.check_consistency(X_toy, y_toy, y_pred_list, conformity_scores)
...
def test_residual_normalised_prefit_conformity_score_get_conformity_scores(
    ...
    conf_scores = residual_norm_conf_score.get_conformity_scores(X_toy, y_toy, y_pred)
...
def test_residual_normalised_conformity_score_get_conformity_scores(y_pred: NDArray) -> None:
    ...
    expected_signed_conf_scores = np.array([np.nan, np.nan, 1.0e08, 1.0e08, 0.0e00, 3.0e08])
    np.testing.assert_allclose(conf_scores, expected_signed_conf_scores)
...
def test_residual_normalised_score_prefit_with_notfitted_estim() -> None:
    ...
    with pytest.raises(ValueError):
        residual_norm_conf_score.get_conformity_scores(X_toy, y_toy, y_pred_list)
...
def test_residual_normalised_score_with_default_params() -> None:
    ...
    conf_scores = residual_norm_score.get_conformity_scores(X_toy, y_toy, y_pred_list)
...
def test_invalid_estimator() -> None:
    ...
    residual_norm_conf_score = ResidualNormalisedScore(residual_estimator=DumbEstimator())
    with pytest.raises(ValueError):
        residual_norm_conf_score.get_conformity_scores(X_toy, y_toy, y_pred_list)
...
def test_cross_residual_normalised() -> None:
    ...
    """
    with pytest.raises(ValueError):
        MapieRegressor(conformity_score=ResidualNormalisedScore()).fit(X_toy, y_toy)
...
def test_residual_normalised_score_pipe() -> None:
    ...
    mapie_reg = MapieRegressor(
        conformity_score=ResidualNormalisedScore(residual_estimator=pipe, split_size=0.2),
        cv="split",
...
def test_residual_normalised_prefit_estimator_with_neg_values() -> None:
    ...
    residual_norm_conf_score = ResidualNormalisedScore(residual_estimator=estim, prefit=True)
...
def test_residual_normalised_prefit_get_estimation_distribution() -> None:
    ...
    residual_normalised_conf_score = ResidualNormalisedScore(residual_estimator=estim, prefit=True)
    conf_scores = residual_normalised_conf_score.get_conformity_scores(X_toy, y_toy, y_pred_list)
    residual_normalised_conf_score.get_estimation_distribution(X_toy, y_pred_list, conf_scores)


@pytest.mark.parametrize(
    "score", [AbsoluteConformityScore(), GammaConformityScore(), ResidualNormalisedScore()]
)
