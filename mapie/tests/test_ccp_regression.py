...
def test_phi_n_attributes(phi: PhiFunction, n_out_raw: int) -> None:
    ...
    assert phi.n_in == 10
    assert phi.n_out == n_out_raw + 1   # +1 because marginal_guarantee=True
...
def test_results_with_constant_sample_weights(
    ...
    np.testing.assert_allclose(y_pred0, y_pred1, rtol=1e-4, atol=1e-2)
    np.testing.assert_allclose(y_pred0, y_pred2, rtol=1e-4, atol=1e-2)
    np.testing.assert_allclose(y_pis0, y_pis1, rtol=1e-4, atol=1e-2)
    np.testing.assert_allclose(y_pis0, y_pis2, rtol=1e-4, atol=1e-2)
