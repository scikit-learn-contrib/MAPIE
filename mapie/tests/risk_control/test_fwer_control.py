...
def test_covert_fwerprocedure_run_break():
    class Dummy(FWERProcedure):
        def _init_state(self, n_lambdas, delta):
            self.calls = 0
        ...
        def _select_next_hypothesis(self, p):
            self.calls += 1
            return self.calls - 1 if self.calls <= 2 else None
...
def test_fixed_sequence_multistart_correct_parsing():
    """Test 3 equally spaced multi-starts: [0, 4, 8]"""
    p_values = np.array(
        [0.03, 0.004, 0.005, 0.25, 0.005, 0.2, 0.8, 0.08, 0.009, 0.005, 0.006, 0.007]
    )
    delta = 0.09
