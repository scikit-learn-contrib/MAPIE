...
STRATEGIES = {
    ...
    "ltt": (
        Params(method="ltt", bound=None, random_state=random_state, metric_control="precision"),
    ),
}
...
def test_valid_estimator() -> None:
    ...
    mapie_clf = MapieMultiLabelClassifier(estimator=clf, random_state=random_state)
...
def test_valid_bound(bound: str) -> None:
    """Test that valid methods raise no errors."""
    mapie_clf = MapieMultiLabelClassifier(random_state=random_state, method="rcps")
    mapie_clf.fit(X_toy, y_toy)
...
def test_predict_output_shape(strategy: str, alpha: Any, delta: Any) -> None:
    ...
    mapie_clf.fit(X, y)
    y_pred, y_ps = mapie_clf.predict(X, alpha=alpha, bound=args["bound"], delta=0.1)
...
def test_results_for_partial_fit(strategy: str) -> None:
    ...
    y_pred, y_ps = mapie_clf.predict(X, alpha=[0.1, 0.1], bound=args["bound"], delta=0.1)
...
def test_results_for_alpha_as_float_and_arraylike(strategy: str, alpha: Any) -> None:
    ...
    y_pred_array, y_ps_array = mapie_clf.predict(X, alpha=alpha, bound=args["bound"], delta=0.9)
...
def test_results_single_and_multi_jobs(strategy: str) -> None:
    ...
    mapie_clf_single = MapieMultiLabelClassifier(
        n_jobs=1, metric_control=args["metric_control"], random_state=args["random_state"]
    )
...
def test_method_error_in_fit(method: str) -> None:
    ...
    mapie_clf = MapieMultiLabelClassifier(random_state=random_state, method=method)
...
def test_metric_error_in_fit(metric_control: str) -> None:
    ...
    mapie_clf = MapieMultiLabelClassifier(random_state=random_state, metric_control=metric_control)
...
def test_error_ltt_delta_null() -> None:
    ...
    mapie_clf = MapieMultiLabelClassifier(random_state=random_state, metric_control="precision")
    mapie_clf.fit(X_toy, y_toy)
...
def test_bound_none_crc() -> None:
    ...
    mapie_clf = MapieMultiLabelClassifier(random_state=random_state, method="crc")
...
def test_partial_fit_second_time():
    clf = MultiOutputClassifier(LogisticRegression()).fit(X, y)
    mclf = MapieMultiLabelClassifier(estimator=clf, random_state=random_state)
...
def test_error_wrong_method_metric_precision(method: str) -> None:
    ...
    """
    clf = MultiOutputClassifier(LogisticRegression()).fit(X_toy, y_toy)
    mapie_clf = MapieMultiLabelClassifier(clf, method=method, metric_control="precision")
...
def test_check_metric_control(method: str) -> None:
    ...
    """
    clf = MultiOutputClassifier(LogisticRegression()).fit(X_toy, y_toy)
    mapie_clf = MapieMultiLabelClassifier(clf, method=method, metric_control="recall")
    with pytest.raises(ValueError, match=r".*Invalid method for metric*"):
        mapie_clf.fit(X_toy, y_toy)
...
def test_method_none_precision() -> None:
    clf = MultiOutputClassifier(LogisticRegression()).fit(X_toy, y_toy)
    mapie_clf = MapieMultiLabelClassifier(clf, metric_control="precision")
    mapie_clf.fit(X_toy, y_toy)
...
def test_method_none_recall() -> None:
    clf = MultiOutputClassifier(LogisticRegression()).fit(X_toy, y_toy)
    mapie_clf = MapieMultiLabelClassifier(clf, metric_control="recall")
    mapie_clf.fit(X_toy, y_toy)
