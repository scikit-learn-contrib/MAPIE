...
def test_results_with_constant_sample_weights(
    ...
    mapie0.fit(
        X_train, y_train, X_calib=X_calib, y_calib=y_calib, sample_weight=None
    )
    mapie1.fit(
        X_train,
        y_train,
    ...
    np.testing.assert_allclose(
        mapie0.conformity_scores_, mapie1.conformity_scores_
    )
    np.testing.assert_allclose(
        mapie0.conformity_scores_, mapie2.conformity_scores_
    )
...
def test_results_for_same_alpha(
    estimator: RegressorMixin, symmetry: bool
) -> None:
    """
...
def test_quantile_prefit_three_estimators() -> None:
    ...
    with pytest.raises(
        ...
        mapie_reg = MapieQuantileRegressor(
            estimator=list_estimators, cv="prefit"
        )
