...
ParamsPredict = TypedDict(
    "ParamsPredict",
    {"include_last_label": Union[bool, str], "agg_scores": str},
)
...
STRATEGIES = {
    ...
    "score_cv_mean": (
        Params(
            method="score", cv=3, test_size=None, random_state=random_state
        ),
        ParamsPredict(include_last_label=False, agg_scores="mean"),
    ),
    "score_cv_crossval": (
    ...
    "raps_split": (
        Params(
            method="raps", cv="split", test_size=0.5, random_state=random_state
        ),
        ParamsPredict(include_last_label=True, agg_scores="mean"),
    ),
    "raps_randomized": (
...
class WrongOutputModel:
    ...
    def predict(self, *args: Any) -> NDArray:
        pred = (self.proba_out == self.proba_out.max(axis=1)[:, None]).astype(
            int
        )
        return pred
...
def test_cumulated_scores() -> None:
    ...
    # predict
    _, y_ps = mapie_clf.predict(
        cumclf.X_test, include_last_label=True, alpha=alpha
    )
...
def test_method_error_in_predict(method: Any, alpha: float) -> None:
    ...
    mapie_clf = MapieClassifier(method="score", random_state=random_state)
    mapie_clf.fit(X_toy, y_toy)
...
def test_include_label_error_in_predict(
    ...
    monkeypatch.setattr(
        MapieClassifier, "_check_include_last_label", do_nothing
    )
    mapie_clf = MapieClassifier(
    ...
    with pytest.raises(ValueError, match=r".*Invalid include.*"):
        mapie_clf.predict(
            X_toy, alpha=alpha, include_last_label=include_labels
        )
...
def test_classes_encoder_same_than_model() -> None:
    ...
    y_mapie = y[indices_remove]
    mapie_clf = MapieClassifier(
        estimator=clf, method="cumulated_score", cv="prefit"
    )
    mapie_clf.fit(X_mapie, y_mapie)
