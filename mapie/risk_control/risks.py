...
def compute_risk_recall_image(
    lambdas: NDArray, y_pred_proba: NDArray, y: NDArray
) -> NDArray:
    """
    In `SemanticSegmentationController` when `metric_control=recall`,
    compute the recall per observation for each different
    thresholds lambdas.

    Parameters
    ----------
    y_pred_proba: NDArray of shape (n_samples, height, width, 1)
        Predicted probabilities for each pixel and each observation.

    y: NDArray of shape (n_samples, height, width)
        True labels.
    ...
    """
    if y_pred_proba.ndim != 4:
        raise ValueError(
            "y_pred_proba should be a 4d array, got an array of shape "
            "{} instead.".format(y_pred_proba.shape)
        )
    if y.ndim != 3:
        raise ValueError(
            "y should be a 3d array, got an array of shape {} instead.".format(
                y_pred_proba.shape
            )
        )
    if not np.array_equal(y_pred_proba.shape, y[:, np.newaxis, ...].shape):
    ...
    # transform y_pred_proba and y to numpy if they are not already
    if not isinstance(y_pred_proba, np.ndarray):
        y_pred_proba = np.array(y_pred_proba)
    if not isinstance(y, np.ndarray):
        y = np.array(y)
    n_lambdas = len(lambdas)
    y_pred_proba_repeat = np.repeat(y_pred_proba, n_lambdas, axis=1)
    lambdas = lambdas[np.newaxis, :, np.newaxis, np.newaxis]
    y_pred_th = (y_pred_proba_repeat > lambdas).astype(int)
    # add new axis to y after the first one
    y_repeat = np.repeat(y[:, np.newaxis, ...], n_lambdas, axis=1)
    
    risks = 1 - (_true_positive_images(y_pred_th, y_repeat) / y.sum(axis=tuple(range(1, y.ndim)))[:, np.newaxis])
    return risks    


def compute_risk_precision_image(
    lambdas: NDArray, y_pred_proba: NDArray, y: NDArray
) -> NDArray:
    """
    In `SemanticSegmentationController` when `metric_control=precision`,
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        risks = 1 - _true_positive_images(y_pred_th, y_repeat) / y_pred_th.sum(
            axis=tuple(range(2, y_pred_th.ndim))
        )
    risks[np.isnan(risks)] = 1  # nan value indicate high risks.    
    return risks


def _true_positive_images(y_pred_th: NDArray, y_repeat: NDArray) -> NDArray:
    """
    ...
    """
    tp = (y_pred_th * y_repeat).sum(axis=(-2, -1))
    return tp
