...
class MapieMultiLabelClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _check_delta(self, delta: Optional[float]):
        ...
        if (not isinstance(delta, float)) and (delta is not None):
            raise ValueError("Invalid delta. " f"delta must be a float, not a {type(delta)}")
    ...
    def _check_estimator(
        ...
        if (estimator is None) and (_refit):
            estimator = MultiOutputClassifier(LogisticRegression(multi_class="multinomial"))
    ...
    def _check_bound(self, bound: Optional[str]):
        ...
        if bound not in self.valid_bounds_:
            raise ValueError("bound must be in ['hoeffding', 'bernstein', 'wsr', ``None``]")
    ...
    def _check_metric_control(self):
        ...
        if self.metric_control not in self.valid_metric_:
            raise ValueError(
                "Invalid metric. "
                "Allowed scores must be in the following list " + ", ".join(self.valid_metric_)
            )
    ...
    def _transform_pred_proba(self, y_pred_proba: Union[Sequence[NDArray], NDArray]) -> NDArray:
        ...
        if isinstance(y_pred_proba, np.ndarray):
            ...
            y_pred_proba_stacked = np.stack(y_pred_proba, axis=0)[:, :, 1]  # type: ignore
    ...
    def partial_fit(
        ...
        if first_call or _refit:
            ...
            if self.metric_control == "recall":
                ...
                partial_risk = compute_risk_precision(self.lambdas, y_pred_proba_array, y)
    ...
    def predict(
        ...
        if self.metric_control == "precision":
            ...
            y_pred_proba_array = (
                y_pred_proba_array > np.array(self.lambdas_star)[np.newaxis, np.newaxis, :]
            )
            ...
            self.lambdas_star = find_lambda_star(self.lambdas, self.r_hat_plus, alpha_np)
            y_pred_proba_array = y_pred_proba_array > self.lambdas_star[np.newaxis, np.newaxis, :]
