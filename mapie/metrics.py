...
def regression_mean_width_score(
    y_pred_low: ArrayLike, y_pred_up: ArrayLike
) -> float:
    """
...
def classification_coverage_score_v2(
    ...
    y_true = np.expand_dims(y_true, axis=1)
    coverage = np.nanmean(
        np.take_along_axis(y_pred_set, y_true, axis=1), axis=0
    )
    return coverage[0]
...
def _gaussian_kernel(x: NDArray, kernel_size: int) -> NDArray:
    ...
    norm_x = x**2
    dist = (
        -2 * np.matmul(x, x.transpose((0, 2, 1)))
        + norm_x
        + norm_x.transpose((0, 2, 1))
    )
...
def spiegelhalter_statistic(y_true: NDArray, y_score: NDArray) -> float:
    ...
    denominator = np.sqrt(
        np.sum((1 - 2 * y_score) ** 2 * y_score * (1 - y_score))
    )
