...
class PhiFunction():
    ...
    used in the Gibbs et al. method to estimate the conformity score
    Phi takes as input X (and can take y_pred and any exogenous variable)
    ...
    marginal_guarantee: bool
        Add a column of ones to ``phi(X, y_pred, z)`` for safety reason
        (to garanty the marginal coverage, no matter the ``functions`` value).
        If the functions covers all the dataset (meaning, for all calibration
        and test samples, ``phi(X, y_pred, z)`` is never all zeros),
    ...
    def __call__(
        ...
        for f in funct_list:
            ...
            if isinstance(f, PhiFunction):
                ...
                res.append(f(disable_marginal_guarantee=True, **used_params))
            else:
                res.append(f(**used_params))
