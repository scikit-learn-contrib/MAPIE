...
from mapie.utils import check_nan_in_aposteriori_prediction, fit_estimator
...
class EnsembleRegressor(EnsembleEstimator):
    ...
    def _fit_oof_estimator(
        ...
        estimator = fit_estimator(estimator, X_train, y_train, sample_weight=sample_weight)
        return estimator
        ...
        if self.cv == "prefit":
            ...
            if self.method == "naive":
                ...
                outputs = Parallel(n_jobs=self.n_jobs, verbose=self.verbose)(
                    ...
                    for (_, calib_index), estimator in zip(cv.split(X), self.estimators_)
                )
