...
def get_r_hat_plus(losses, bound, delta, sigma_init=.25):
    ...
    assert (
        bound in ["hoeffding", "bernstein", "wsr"]
    ), 'bounds must be in ["hoeffding", "bernstein", "wsr"]'
    ...
    if bound == "hoeffding":
        ...
        r_hat_plus["bernstein"] = (
            r_hat + np.sqrt((sigma_hat_bern * 2 * np.log(2 / delta)) / n_obs) +
        ...
        for batch, n_batch in batches.item():
