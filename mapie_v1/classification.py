...
class SplitConformalClassifier:
    ...
    def __init__(
        ...
        # 'simple' (provide test_size in .fit) or 'prefit'.
        # Future API: 'manual' (provide X_calib, Y_calib in .fit)
        # and BaseCrossValidator (restricted to splitters only)
    ...
    def fit(
        ...
        # Removed, because it is not used in split conformal classifier
        test_size: Union[int, float] = 0.1,  # -> In __init__ ?
        fit_params: Optional[dict] = None,
        # For example, LBGMClassifier :  {'categorical_feature': 'auto'}
        predict_params: Optional[dict] = None,
        # For example, LBGMClassifier :  {'pred_leaf': False}
...
class CrossConformalClassifier:
    ...
    def predict_sets(
            ...
            conformity_score_params: Optional[dict] = None) -> NDArray:
        """
        Return
        -----
        An array containing the prediction sets
