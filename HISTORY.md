# History
## 1.x.x (2026-xx-xx)
- Add `ConditionalRiskController` (experimental, PyTorch-based) in `mapie.risk_control.adaptive_conformal_risk_control`, which controls a bounded monotone differentiable loss with a learned, input-dependent prediction parameter. Built-in recall and symmetric-interval miscoverage losses are selected by their preferred string names; custom losses use `RiskLoss`, whose `higher_is_better` argument distinguishes performance metrics from losses. Also add a reproduction of Figure 2 from Blot et al. (2025). The controller is exported from `mapie.risk_control` but loaded lazily, so it does not make PyTorch a hard dependency of the core package; importing it raises an actionable error pointing at the `conditional` extra if PyTorch is missing.
- Fix `ConditionalRiskController` mini-batch training so each batch is an unbiased estimate of the full AA-CRC objective, including incomplete final batches.
...
- Expose `p0_p1_output` in `VennAbersCalibrator.predict_proba`.
...
- Extended risks to `ContinuousRisk` for more general risk monitoring.
...
- Refactor offline tests and simplify parts of regression internals (including `sample_weight` handling and moving `_check_gamma` to `time_series_regression.py`).

### Bug fixes

- Fix `sample_weight` routing through scikit-learn `Pipeline` in `_fit_estimator`.
...
- Simplify release checklist: migrate the release flow to `setuptools-scm` , smoke tests in the workflows, use release candidates for TestPypi.
...
- Created a HuggingFace repository for MAPIE: [https://huggingface.co/datasets/mapie-library](https://huggingface.co/datasets/mapie-library)
